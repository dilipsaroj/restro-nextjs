import { times } from "../../data";
import { PrismaClient, Table } from "@prisma/client";
import { NextApiResponse } from "next";

const prisma = new PrismaClient()

export const findAvailableTable = async ({ time, day, res, restaurant }: {
    time: string, day: string, res: NextApiResponse,
    restaurant: {
        tables: Table[];
        open_time: string;
        close_time: string
    }
}) => {
    const searchTimes = times.find((item) => {
        return item.time === time
    })?.searchTimes;

    if (!searchTimes) {
        return res.status(400).json({ errorMessage: 'invalid data provided' })
    }

    const bookings = await prisma.booking.findMany({
        where: {
            booking_time: {
                gte: new Date(`${day}T${searchTimes[0]}`),
                lte: new Date(`${day}T${searchTimes[searchTimes.length - 1]}`)
            }
        },
        select: {
            number_of_people: true,
            booking_time: true,
            tables: true
        }
    })

    const bookingTablesObj: { [key: string]: { [key: number]: true } } = {}

    bookings.forEach(item => {
        bookingTablesObj[item.booking_time.toISOString()] = item.tables.reduce((obj, table) => {
            return {
                ...obj,
                [table.table_id]: true
            }
        }, {})
    })

    const tables = restaurant.tables;

    const searchTimesWithTable = searchTimes.map((item) => {
        return {
            date: new Date(`${day}T${item}`),
            time: item,
            tables
        }
    })

    searchTimesWithTable.forEach((item) => {
        item.tables = item.tables.filter(table => {
            if (bookingTablesObj[item.date.toISOString()]) {
                if (bookingTablesObj[item.date.toISOString()][table.id]) return false;
            }
            return true;
        })
    })

    return searchTimesWithTable;
}